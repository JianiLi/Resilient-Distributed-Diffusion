...
for F = -1:1:numF
    ...
    elseif F == 0
        ...
        hold on;
        set(gca,'FontSize',15);
        mylgd{3} = ['DLMSAW/R-DLMSAW/general R-DLMSAW, F = ', num2str(F)];
    else 
        if algorithm == 1
            plot(mag2db(MSD_coop),'linewidth',1, 'Color', [0.2,0.5+0.1*F,0.2+0.1*F]);
            mylgd{2*F+1+algorithm} = ['R-DLMSAW, F = ', num2str(F)];
        else
            plot(mag2db(MSD_coop),'--', 'linewidth',1, 'Color',[0.5+0.1*F,0.1*F,0.3]);
            mylgd{2*F+1+algorithm} = ['general R-DLMSAW, F = ', num2str(F)];
        end
        hold on;
        set(gca,'FontSize',15);
    end
