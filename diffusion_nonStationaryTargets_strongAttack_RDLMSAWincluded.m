...
%    Expectation_noco, Expectation_coop, gamma2 );
   ...
   [newAdjacency,ratio,J] = removeLargest_new_resilient(n, F, newAdjacency, numAgents, Adjacency, attacker, D, U, phi, storedNum, attacker_phi, ...
    ...
    if n <= numPoints/2 & mod(n,500)==0
        AdjacencyMatrix = (A>=0.0001) & newAdjacency;
    elseif mod(n,500)==0
        AdjacencyMatrix = (A>=0.1) & newAdjacency;
    end
...
%         end
    ...
    wAverageMatrix = getwAverage(numAgents, wAverageMatrix, attacker, newAdjacency, w);
end
...
set(h1,'interpreter','Latex','FontSize',22);
set(h2,'interpreter','Latex','FontSize',22);
set(gca,'TickLabelInterpreter','latex', 'FontSize',15);
set(gcf,'color','white')
%xlabel('','fontsize',25);ylabel('Y','fontsize',25);
%h=legend('Attacker selected state $w_{k,i}^a$','Average state $\bar{w}_{k,i}$ of attacked nodes');
h=legend('$w_{k,i}^a$','$\bar{w}_{k,i}$');
set(h,'interpreter','Latex','FontSize',22)
xlabel('State $w_{k,i}(1)$', 'interpreter','latex','fontsize',25);
ylabel('State $w_{k,i}(2)$', 'interpreter','latex','fontsize',25);
% figure(4)
% D = [];
% for k = 1:numPoints-1
