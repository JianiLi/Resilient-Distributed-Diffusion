...
for n = numTaps : numPoints
    ...
    if n >numTaps
        A_max = A;
        J_max = 100*ones(1,numAgents); 
        J_w = 100*zeros(1,numAgents); 
         for F = 0:5
            newAdjacency = Adjacency;
     ...
     Expectation_noco, Expectation_coop, gamma2 );
        ...
        w = UpdateW(attackers, numAgents, A_max, w,  phi, attacker_phi, w0_attacker);
    end
...
%             for k = 1:numAgents
%                 J_new(k) = sum((D(:,k) - U(:,:,k) * w_test(:,k)).^2)/storedNum;
%                 if J_new(k) < J_max(k)
%                     J_max(k) = J_new(k);
%                     w_new(:,k) = w_test(:,k);
%                 end
